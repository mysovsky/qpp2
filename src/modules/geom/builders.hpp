...
namespace qpp {
  ...
  template<class REAL>
  void treat_crowd(geometry<REAL> & geom, int mode) {
    ...
    neighbours_table<REAL> n(geom,b);
    n.build();
    std::vector<int> todel(geom.nat(), -1);
    ...
    xgeometry<REAL> *xgeom = dynamic_cast<xgeometry<REAL>*>(&geom);
  ...
  void copy_header(geometry<REALDST> & dst,
                   const geometry<REALSRC> & src) {

    xgeometry<REALDST> *xdst = nullptr;
  ...
  */
  ...
  template<class REAL>
  void fill( geometry<REAL> & dst,
             const geometry<REAL> & src,
    ...
    std::vector<std::vector<typename xgeometry<REAL>::fieldtypes >> v(src.nat());
    std::vector<vector3<REAL> > r1(src.nat());
        ...
        if (something)
          for (int at=0; at<src.nat(); at++)
            if (!src.shadow(at) && (allcell || inside[at])) {
                dst.add("",vector3<REAL>::Zero());
                dst.set_fields(dst.nat()-1,v[at]);
		dst.coord(dst.nat()-1) = r1[at];
              }
      }
  ...
  void fill( geometry<REAL> & dst,
    ...
    //    if (typeid(CELL)==typeid(periodic_cell<REAL>)) {

        std::cout << "fillf:: alive1\n";

        // Minimal and maximal fractional coordinates of the shape
        //periodic_cell<REAL> * cell = (periodic_cell<REAL>*)(&src.cell);
        auto cell = src.cell;
        vector3<REAL> Smin = shp.fmin(*cell),
            Smax = shp.fmax(*cell);

        std::cout << "Smin= " << Smin << " Smax= " << Smax << "\n";

        // Minimal and maximal fractional coordinates of atoms
        vector3<REAL> fmin,fmax;
        for (int at=0; at<src.nat(); at++) {
            vector3<REAL> f = src.coord(at);
  ...
  template<class REAL>
  std::set<int> select( const geometry<REAL> & geom,
                        const shape<REAL> & shp) {
    return std::set<int>();
  }
  ...
  template<class REAL>
  std::set<int> select( const geometry<REAL> & geom,
                        const geometry<REAL> & frag, REAL R) {
    bonding_table<REAL> b;
  ...
  */
  ...
  void unique(std::vector<int> & n_images,
              geometry<REAL> & ugeom,
              const geometry<REAL> & nugeom,
              const SYMM & group,
              const index & begin,
              const index & end,
              const std::function<REAL(const geometry<REAL> &, int)> & key,
  ...
  void unique(std::vector<int> & n_images,
              ...
              SYMM & group,
              const std::function<REAL(const geometry<REAL> &, int)> & key =
              [](const geometry<REAL> &g, int i) -> REAL
  ...
  template<class REAL, class SYMM>
  void unique(geometry<REAL> & ugeom,
              const geometry<REAL> & nugeom,
              const SYMM & group,
              const std::function<REAL(const geometry<REAL> &, int)> & key =
              [](const geometry<REAL> &g, int i) -> REAL
              { return std::sqrt(g.y[i]*g.y[i]+g.z[i]*g.z[i]); },
              REAL eps = geometry<REAL>::tol_geom) {
  ...
  */
  template<class REAL>
  void py_fill1( geometry<REAL> & dst,
                 const geometry<REAL> & src,
                 const shape<REAL> & shp, const periodic_cell<REAL> & cell,
                 const index & begin, const index & end, int mode)
  {  fill(dst,src,shp,cell,begin,end,mode); }

  template<class REAL>
  void py_fill2( geometry<REAL> & dst,
                 const geometry<REAL> & src,
                 const shape<REAL> & shp, int mode)
  {  fill(dst,src,shp,mode); }

  template<class REAL>
  void py_fill3( geometry<REAL> & dst,
                 const geometry<REAL> & src,
                 const shape<REAL> & shp)
  {  fill(dst,src,shp); }
  ...
  void py_unique3(py::list & images,
                  ...
                  REAL eps) {
    std::vector<int>  n_images;
    unique(n_images, ugeom, nugeom, *ugeom.cell, key, eps);
    images.attr("clear")();
    for (const auto & n : n_images)
      images.append(n);
  }
  ...
  template<class REAL, class SYMM>
  void py_unique4(geometry<REAL> & ugeom,
                  const geometry<REAL> & nugeom,
                  const SYMM & group,
                  const std::function<REAL(const geometry<REAL> &, int)> & key,
                  REAL eps)
  ...
  template<class REAL>
  void py_unique5(geometry<REAL> & ugeom,
                  const geometry<REAL> & nugeom,
                  const std::function<REAL(const geometry<REAL> &, int)> & key,
                  REAL eps)
