...
namespace qpp {
  ...
  /// \param geom
  template<class REAL>
  void read_xyz(std::basic_istream<CHAR_EX,TRAITS> & inp, geometry<REAL> & geom) {
	...
	(*geom.cell)(2) = vector3<REAL>(std::stof(splt[6].data()),
      ...
      if (splt.size() == 6) {
	geom.cell = std::make_shared<periodic_cell<REAL> >(std::stof(splt[0].data()), std::stof(splt[1].data()),
							   std::stof(splt[2].data()), std::stof(splt[3].data()),
							   std::stof(splt[4].data()), std::stof(splt[5].data()));
      }
  ...
  void read_xyzq(std::basic_istream<CHAR_EX,TRAITS> & inp,
                 xgeometry<REAL> & geom,
                 bool preserve_geom = false, int xgeom_charge_field_idx = 0) {
    ...
    xgeom_charge_field_idx = geom.findex("charge");
    ...
    for (int i=0; i<geom.nat(); i++)
      out <<
             fmt::format("{:8s} {:12.6f} {:12.6f} {:12.6f} {:12.6f}\n",
                         ...
                         geom.coord(i)(2),  geom.template xfield<REAL>(geom.findex("charge"),i));
  }
  // -------------------------------------------------------------------//
