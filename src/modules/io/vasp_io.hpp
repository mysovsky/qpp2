...
namespace qpp {
  ...
  void write_vasp_poscar(std::basic_ostream<CHAR_EX,TRAITS>  & out,
    ...
    for (int i = 0; i < geom.typetable()->n_types(); i++)
      poscar_comment += geom.typetable()->atomic_type(i) + (i == geom.typetable()->n_types() - 1 ? "" : " ");
    fmt::print(out, "{}\n", poscar_comment);
    ...
    for (int i = 0; i < geom.typetable()->n_types(); i++)
      fmt::print(out, i == geom.typetable()->n_types() - 1 ? "{}" : "{} ", geom.typetable()->atom_count_by_type(i));
