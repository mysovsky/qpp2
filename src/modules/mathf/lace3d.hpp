...
template <typename ELEM> using matrix = Eigen::Matrix<ELEM, Eigen::Dynamic, Eigen::Dynamic>;
  /*
  std::string fnumber(float x){
    return fmt::format(" {:12.8f}",x);
  }
  
  std::string fnumber(double x){
    return fmt::format(" {:12.8f}",x);
  }
  
  std::string fnumber(std::complex<float> x){
    std::string res= fmt::format(" {:12.8f}",x.real());
    if (x.imag()>0)
      res.append("+");
    res = res + fmt::format(" {:12.8f}",x.imag());
    res.append("i");
    return res;
  }
  
  
  std::string fnumber(std::complex<double> x){
    std::string res= fmt::format("{:12.8f}",x.real());
    if (x.imag()>0)
      res=res.append("+");
    res = res + fmt::format("{:12.8f}",x.imag());
...
std::vector<int> LI_vectors(const MTR &M, VALTYPE eps) {
  ...
  std::vector<int> LI;
  matrix<VALTYPE> P = matrix<VALTYPE>::Zero(M.rows(), M.rows());

  for (int i=0; i<M.cols(); i++){
    Eigen::Matrix<VALTYPE, Eigen::Dynamic, 1> orth = M.col(i) - P*M.col(i);
    //orth -=  P*M.col(i);
    VALTYPE s = orth.norm();
    if (s < eps)
      continue;
