...
namespace qpp {
  ...
  */
  ...
  struct periodic_cell {
    ...
    std::shared_ptr<REAL> tol_transl, tol_rot;
    ...
    //!  translation vectors
    periodic_cell (int dim = 0): tol_transl(std::make_shared<REAL>(tol_transl_default)),
				 tol_rot(std::make_shared<REAL>(tol_rot_default))
    {
      DIM = dim;
      //      *tol_transl = ;
      //*tol_rot = ;
    ...
    periodic_cell (const periodic_cell<REAL> & cl):  tol_transl(std::make_shared<REAL>(tol_transl_default)),
						     tol_rot(std::make_shared<REAL>(tol_rot_default))
    {
      DIM = cl.DIM;
      v = cl.v;
    ...
    */
    periodic_cell (REAL a, REAL b, REAL c, REAL alpha, REAL beta, REAL gamma):
      tol_transl(std::make_shared<REAL>(tol_transl_default)),
      tol_rot(std::make_shared<REAL>(tol_rot_default))
    {
      DIM = 3;
      v.resize(3);
  ...
  template<class REAL>
  REAL periodic_cell<REAL>:: tol_transl_default=1e-6;
  template<class REAL>
  REAL periodic_cell<REAL>:: tol_rot_default=1e-6;
